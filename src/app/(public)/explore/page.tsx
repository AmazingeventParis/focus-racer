...
export default function ExplorePage() {
  ...
  return (
    <main className="bg-white min-h-screen">
          ...
          <div className="text-center max-w-3xl mx-auto">
            ...
            <div className="max-w-xl mx-auto relative">
              ...
              <Input
                ...
                className="pl-12 pr-4 bg-gray-100 backdrop-blur-sm border-neon-border shadow-lg h-14 rounded-2xl text-gray-900 placeholder:text-gray-400 text-base"
              />
              {searchQuery && (
                <button
                ...
                >
                  <svg className="w-3 h-3 text-gray-800" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
      ...
      <section className="py-2 bg-gray-50">
        <div className="container mx-auto px-4">
          <div className="flex flex-wrap items-center justify-center gap-x-10 gap-y-3 text-center">
            ...
            <div className="w-px h-6 bg-gray-200 hidden sm:block" />
            <div>
              <span className="text-xl font-bold text-neon-cyan">{stats.upcomingCount}</span>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button
                ...
                className={`px-4 py-1.5 rounded-full text-sm font-medium transition-all duration-200 ${
                  selectedSport === "all"
                    ? "bg-neon-cyan text-neon-bg shadow-sm shadow-neon-glow"
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <input
                  ...
                  className={`px-4 py-1.5 rounded-full text-sm font-medium border transition-all duration-200 cursor-pointer ${
                    selectedDate
                      ? "bg-neon-cyan text-neon-bg border-neon-cyan shadow-sm shadow-neon-glow"
          ...
          {isLoading ? (
            ...
            <div className="text-center py-16">
              ...
              {hasActiveFilters && (
                <button
                  onClick={resetFilters}
                  className="px-6 py-2.5 bg-neon-cyan text-neon-bg rounded-xl font-medium hover:bg-neon-cyan/80 transition-colors"
                >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
              {filteredEvents.map((event, i) => {
                ...
                return (
                  ...
                  >
                    <Card className="rounded-xl overflow-hidden hover:shadow-card-hover transition-all duration-300 h-full border-gray-200 hover:border-neon-cyan/40 hover:-translate-y-0.5 bg-white">
                      {event.coverImage ? (
                        ...
                        <div className="aspect-[3/4] flex flex-col items-center justify-center bg-gradient-to-br from-gray-50 to-gray-100 border-b border-gray-200 relative">
                          <span className="text-4xl mb-2 opacity-60">{SPORT_ICONS[event.sportType] || "\u{1F3C5}"}</span>
                          <span className="text-xs font-bold text-gray-800 text-center px-3 leading-tight">
                            {event.name}
                          </span>
                          {isPast && (
                            <div className="absolute top-2 right-2 flex flex-col items-end gap-1">
                              {daysLeft !== null && (
                                <span className={`px-1.5 py-0.5 text-[9px] rounded-full font-medium ${daysLeft <= 7 ? "bg-red-500/80 text-white" : "bg-amber-500/80 text-white"}`}>
                                  {daysLeft}j restant{daysLeft > 1 ? "s" : ""}
                                </span>
                              )}
                              <span className="px-1.5 py-0.5 bg-gray-300 text-gray-800 text-[9px] rounded-full font-medium">Terminé</span>
                            </div>
                          )}
                        </div>
                      )}
                      <div className="p-2.5">
                        <h3 className="text-sm font-semibold text-gray-900 group-hover:text-neon-cyan transition-colors line-clamp-1">{event.name}</h3>
                        ...
                        <div className="flex flex-wrap gap-1 mt-1.5">
                          <Badge variant="outline" className="border-neon-border text-neon-cyan text-[10px] px-1.5 py-0">
                            {SPORT_ICONS[event.sportType]} {SPORT_LABELS[event.sportType] || event.sportType}
                          </Badge>
                          <Badge className="bg-gray-100 text-neon-cyan hover:bg-gray-100 text-[10px] px-1.5 py-0">
      ...
      <section id="comment-ca-marche" data-reveal className="py-16 bg-gray-50 border-t border-gray-100">
        <div className="container mx-auto px-4">
          <div className={`text-center max-w-3xl mx-auto mb-12 transition-all duration-700 ${reveal("comment-ca-marche") ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-gray-100 text-neon-cyan text-sm font-medium mb-4">
              Simple comme 1-2-3
            </div>
          ...
          <div className="grid md:grid-cols-3 gap-8 max-w-4xl mx-auto">
            {[
              ...
              >
                <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-neon-cyan to-neon-purple flex items-center justify-center text-3xl mx-auto mb-5 shadow-lg shadow-neon-glow">
                  {step.icon}
                </div>
                <div className="absolute -top-1 left-1/2 -translate-x-1/2 w-6 h-6 rounded-full bg-gray-100 border-2 border-neon-cyan flex items-center justify-center text-xs font-bold text-neon-cyan">
                  {step.num}
                </div>
      ...
      <section id="sports" data-reveal className="py-12 bg-gradient-to-br from-neon-bg via-neon-bg-secondary to-neon-bg overflow-hidden">
        <div className="container mx-auto px-4">
          <div className={`text-center mb-8 transition-all duration-700 ${reveal("sports") ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
            ...
            <p className="text-gray-500 text-sm">Photos de course à pied, trail, triathlon, cyclisme, natation et bien plus encore.</p>
          </div>
          <div className="flex flex-wrap justify-center gap-2.5 max-w-4xl mx-auto">
            {[
              ...
              <span
                key={i}
                className={`px-4 py-2 rounded-full bg-white/10 backdrop-blur-sm border border-neon-border text-gray-900 text-sm font-medium hover:bg-neon-cyan/20 hover:border-neon-cyan/50 transition-all duration-300 cursor-default ${
        ...
        <div className="container mx-auto px-4 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            <Link href="/solutions/photographes">
              <button className="px-6 py-3 bg-neon-cyan hover:bg-neon-cyan/80 text-neon-bg font-semibold rounded-xl transition-all duration-300 shadow-md shadow-neon-glow hover:-translate-y-0.5">
                Je suis photographe
              </button>
            </Link>
            <Link href="/solutions/organisateurs">
              <button className="px-6 py-3 bg-gray-100 hover:bg-gray-200 text-gray-900 font-semibold rounded-xl border border-neon-border transition-all duration-300 hover:-translate-y-0.5">
