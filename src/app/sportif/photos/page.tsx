...
function ShareBar({ eventName, eventId }: { eventName: string; eventId: string }) {
  ...
  return (
    <div className="flex items-center gap-2 mb-3 overflow-x-auto scrollbar-hide">
      <span className="text-xs text-gray-500 mr-1 flex-shrink-0">Partager :</span>
      {/* Facebook */}
      <a
        href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareUrl)}`}
        target="_blank"
        rel="noopener noreferrer"
        className="w-8 h-8 rounded-full bg-[#1877F2] flex items-center justify-center text-white hover:opacity-80 transition-opacity flex-shrink-0"
        title="Facebook"
      >
      ...
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        className="w-8 h-8 rounded-full bg-black flex items-center justify-center text-white hover:opacity-80 transition-opacity flex-shrink-0"
        title="X (Twitter)"
      >
      ...
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        className="w-8 h-8 rounded-full bg-[#25D366] flex items-center justify-center text-white hover:opacity-80 transition-opacity flex-shrink-0"
        title="WhatsApp"
      >
        <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
      ...
      <button
        ...
        className="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center text-gray-600 hover:bg-gray-300 transition-colors flex-shrink-0"
      ...
      {canNativeShare && (
        <button
          ...
          className="w-8 h-8 rounded-full bg-emerald-500 flex items-center justify-center text-white hover:bg-emerald-600 transition-colors flex-shrink-0"
          title="Partager"
        >
          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
          ...
          {filteredGroups.map((group) => {
            ...
            return (
              <div key={group.event.id}>
                ...
                {/* Share bar + Photos grid */}
                {!isCollapsed && (
                  <>
                  <ShareBar eventName={group.event.name} eventId={group.event.id} />
        ...
        >
          ...
          <div className="absolute bottom-0 left-0 right-0 z-10 bg-gradient-to-t from-black/70 to-transparent p-4 sm:p-6">
            <div className="flex items-center justify-between max-w-3xl mx-auto">
              <div>
                <p className="text-white text-sm font-medium">{lightboxGroup?.event.name}</p>
