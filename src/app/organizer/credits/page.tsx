...
export default function CreditsPage() {
  ...
  useEffect(() => {
    if (searchParams.get("success") === "true") {
      ...
      window.history.replaceState({}, "", "/organizer/credits");
            ...
            <CardContent className="p-6">
              ...
              {subscription?.hasSubscription && subscription.status === "active" && subscription.nextInvoiceDate && (
                <p className="text-emerald-100 text-xs mt-3 flex items-center gap-1.5">
                  ...
                  Prochaine recharge le {new Date(subscription.nextInvoiceDate).toLocaleDateString("fr-FR", { day: "numeric", month: "long" })} à {new Date(subscription.nextInvoiceDate).toLocaleTimeString("fr-FR", { hour: "2-digit", minute: "2-digit" })}
                </p>
              )}
              {isTestMode && (
