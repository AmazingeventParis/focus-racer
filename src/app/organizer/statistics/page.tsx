...
const SPORT_COLORS: Record<string, string> = {
  RUNNING: "bg-emerald-500/100",
  TRAIL: "bg-amber-500/100",
  TRIATHLON: "bg-blue-500/100",
  CYCLING: "bg-rose-500",
  SWIMMING: "bg-cyan-500",
  OBSTACLE: "bg-purple-500/100",
  OTHER: "bg-gray-400",
};

const STATUS_LABELS: Record<string, { label: string; className: string }> = {
  PUBLISHED: { label: "Publié", className: "bg-emerald-500/100/10 text-emerald-400" },
  DRAFT: { label: "Brouillon", className: "bg-gray-500/10 text-gray-400" },
  ARCHIVED: { label: "Archivé", className: "bg-teal-500/100/10 text-teal-400" },
};
...
export default function StatisticsPage() {
  ...
  const fetchStats = useCallback(async () => {
    try {
      setIsLoading(true);
      const res = await fetch("/api/stats/organizer");
      if (!res.ok) {
  ...
  return (
    <div className="p-4 md:p-8 animate-fade-in max-w-7xl mx-auto">
      ...
      {/* Tabs */}
      <div className="flex gap-1 bg-[#F1F5F9] rounded-xl p-1 mb-8 overflow-x-auto">
        {tabs.map(tab => (
          <button
            key={tab.id}
            ...
            className={`flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 whitespace-nowrap ${
              activeTab === tab.id
                ? "bg-[#FFFFFF] text-[#059669] shadow-sm"
                ...
                {isLoading ? (
                  ...
                  <div className="space-y-4">
                    <div className="p-4 rounded-xl bg-gradient-to-r from-emerald-500/10 to-teal-500/10">
                      ...
                      <p className="text-3xl font-bold font-display text-emerald-400">{stats?.credits.balance || 0}</p>
        ...
        <div className="space-y-8">
          ...
          <Card className="bg-white border-0 shadow-card rounded-2xl">
            ...
            <CardContent className="space-y-3">
              <div className="flex items-start gap-3 p-4 rounded-xl bg-emerald-500/10">
                <svg className="w-5 h-5 text-emerald-400 mt-0.5 flex-shrink-0" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
              ...
              <div className="flex items-start gap-3 p-4 rounded-xl bg-purple-500/10">
                <svg className="w-5 h-5 text-purple-400 mt-0.5 flex-shrink-0" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
        ...
        <div className="space-y-8">
          ...
          <Card className="bg-white border-0 shadow-card rounded-2xl">
            <CardHeader className="flex flex-row items-center justify-between">
              ...
              <Badge className="bg-gray-500/10 text-gray-400">{events.length} total</Badge>
            </CardHeader>
            <CardContent>
              {isLoading ? (
                ...
                <div className="space-y-2">
                  {events.map(event => {
                    ...
                    return (
                      <div key={event.id} className="flex items-center gap-4 p-3 rounded-xl hover:bg-[#F1F5F9] transition-colors">
      ...
      <div className="mt-8 flex justify-center">
        <Button
          ...
          onClick={fetchStats}
          disabled={isLoading}
          className="text-gray-500 hover:text-emerald-400 border-[#E2E8F0] hover:border-emerald-500/30 rounded-xl"
        >
          {isLoading ? (
