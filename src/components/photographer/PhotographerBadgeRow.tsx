...
import { PHOTOGRAPHER_BADGE_DEFINITIONS } from "@/lib/photographer-badges";
...
export default function PhotographerBadgeRow() {
  const [badges, setBadges] = useState<EarnedBadge[]>([]);
  const [newlyEarned, setNewlyEarned] = useState<string[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch("/api/photographer/badges")
      .then((res) => res.json())
      .then((data) => {
  ...
  if (loading) {
    return (
      <div className="bg-white border-0 shadow-card rounded-xl p-6">
      ...
      <div className="flex items-center justify-between mb-5">
        <h2 className="text-lg font-display font-semibold text-gray-900">
          ...
          <span className="text-sm font-normal text-gray-500">
            ({earnedKeys.size}/{PHOTOGRAPHER_BADGE_DEFINITIONS.length})
          </span>
        </h2>
      </div>

      <div className="grid grid-cols-3 sm:grid-cols-5 gap-4">
        {PHOTOGRAPHER_BADGE_DEFINITIONS.map((def) => {
