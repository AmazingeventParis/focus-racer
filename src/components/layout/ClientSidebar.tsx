...
interface NavItem {
  ...
  roles?: string[]; // If undefined, visible for all roles
  disabled?: boolean;
}
...
export default function ClientSidebar() {
  ...
  useEffect(() => {
    if (pathname === "/photographer/orders" || pathname.startsWith("/photographer/orders/")) {
  ...
  return (
    <aside className="hidden md:flex w-64 bg-white border-r border-slate-200 shadow-sidebar text-slate-900 min-h-screen flex-col">
      ...
      <div className="p-6">
        ...
        <p className="text-slate-500 text-xs mt-2 ml-10">{getRoleLabel(userRole)}</p>
      </div>
        ...
        <ul className="space-y-1">
          {navItems.map((item) => {
            ...
            if (item.disabled) {
              return (
                <li key={item.href}>
                  <span className="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium text-slate-400 cursor-not-allowed select-none">
                    <span className="text-slate-300">{item.icon}</span>
                    {item.labelKey ? t(item.labelKey) : item.label}
                    <span className="ml-auto text-[10px] uppercase tracking-wide text-slate-400 font-semibold">Nous contacter</span>
                  </span>
                </li>
              );
            }
            return (
              <li key={item.href}>
                ...
                >
                  <span className={cn(isActive ? "text-emerald" : "text-slate-400")}>
                    {item.icon}
                  </span>
                  {item.labelKey ? t(item.labelKey) : item.label}
